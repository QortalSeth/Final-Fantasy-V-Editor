...
export const CheckBoxWithDefault = React.forwardRef<CheckboxRef, CheckBoxWithDefaultProps>(
  ({ labelStyle = {}, labelText }: CheckBoxWithDefaultProps, ref) => {
    ...
    return (
      <div>
        <label htmlFor={labelText} style={{ ...labelStyle }} className='noselect'>
          <input
            type='checkbox'
            id={labelText}
            checked={checked}
          ...
          />
          <input type='checkbox' id='' checked={defaultChecked} ref={defaultRef} disabled style={{ marginRight: '2vw' }} />
          <span>{labelText}</span>
        </label>
      </div>
    );
